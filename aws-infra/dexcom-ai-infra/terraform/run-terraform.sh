...
eval $(bw get item "Dexcom-AWS-Variables" | jq -r '.fields[] | select(.value == null or .value == "") | .name')
eval $(bw get item "Dexcom-CloundFlare" | jq -r '.fields[] | select(.value == null or .value == "") | .name')
